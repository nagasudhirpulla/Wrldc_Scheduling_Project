...
exports.get = function (seller_id, buyer_id, trader_id, transaction_type_id, date, revision, done) {
    ...
    db.get().query('SELECT timeblock, value FROM desired_schedules WHERE seller_id = ? AND buyer_id = ? AND trader_id = ? AND transaction_type_id = ? AND date = ? AND revision = ?', values, function (err, rows) {
        if (err) return done(err);
        done(null, rows);
    });
};