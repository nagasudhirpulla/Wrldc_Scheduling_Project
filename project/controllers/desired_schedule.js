...
var Desired_schedule = require('../models/desired_schedule.js');
...
router.get('/getforrevision', function (req, res) {
    ...
    Desired_schedule.getForRevision(seller_id, buyer_id, trader_id, transaction_type_id, new Date(req.query.date), revision, function (err, rows) {
        if (err) {
            res.send({'Error': err});
        }
        res.json({'desired_schedules': rows});
    });
});
